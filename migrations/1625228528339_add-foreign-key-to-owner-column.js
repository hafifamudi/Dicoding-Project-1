...
exports.up = pgm => {
  ...
  pgm.sql("UPDATE music SET owner = 'old_notes' WHERE owner = NULL");
 
  // memberikan constraint foreign key pada owner terhadap kolom id dari tabel users
  pgm.addConstraint('music', 'fk_music.owner_users.id', 'FOREIGN KEY(owner) REFERENCES users(id) ON DELETE CASCADE');
};

exports.down = pgm => {
  // menghapus constraint fk_notes.owner_users.id pada tabel notes
  pgm.dropConstraint('music', 'fk_notes.owner_users.id');
 
  // mengubah nilai owner old_notes pada note menjadi NULL
  pgm.sql("UPDATE music SET owner = NULL WHERE owner = 'old_notes'");
